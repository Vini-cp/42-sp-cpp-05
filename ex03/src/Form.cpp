...
void Form::setIsSigned( bool pSignValue )
{
    mIsSigned = pSignValue;
}
...
bool Form::checkRequirementsToExecute( const Bureaucrat& prBureaucrat ) const
{
    if ( mIsSigned && prBureaucrat.getGrade() <= mGradeToExecute )
    {
        return true;
    }
    else if ( !mIsSigned )
